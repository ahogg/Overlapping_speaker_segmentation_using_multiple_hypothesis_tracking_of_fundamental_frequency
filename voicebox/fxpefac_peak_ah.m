function [fx,tx,pv,fv]=fxpefac_peak_ah(s,fs,tinc,m,pp)
...
p.fmax=1500;            % maximum frequency of initial spectrogram (Hz)
p.fres = 20;            % bandwidth of initial spectrogram (Hz)
p.fbanklo = 10;         % low frequency limit of log filterbank (Hz)
p.mpsmooth = 71;       % width of smoothing filter for mean power
...
p.numopt = 16;           % number of possible frequencies per frame
p.flim = [70 1500];      % range of feasible fundamental frequencies (Hz)
...
p.sopt = 'pJc';      % spectrogram options
...
% Middle
sca = sca(sca<1.1 & sca>0.9);  % restrict to 0.5 - 10.5 times fundamental
...
if ~nargout || any(m=='g') || any(m=='G')
    ...
    if any(m=='G') || ~nargout && ~any(m=='g')
        clf;
        spgrambw(s,fs,p.sopt, 30, [0 8 1500]); % draw spectrogram with log axes
        hold on
        plot(tx,fxg,'-c',tx,fxb,'-r'); % fx track
        y=get(gca,'ylim');
